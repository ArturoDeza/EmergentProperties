...
Distortion_Family = ['Window_Cue_Conflict']
		...
		for z in range(8):
			...
			with torch.no_grad():
				for data in testloader:
					...
					labels_faux = labels.clone() + 1
					labels_faux[labels_faux==20] = 0 # remap the overflowing 20 label to 0
					...
					#for i in range(testing_batch_size):
						label = labels[i]
						if len(labels)>1:
							class_periphery_Object[label] += c[i].item()
							class_fovea_Object[label] += c_faux[i].item()
							class_total_Object[label] += 1
						else:
							class_periphery_Object[label] += c.item()
							class_fovea_Object[label] += c_faux.item()
							class_total_Object[label] += 1
			del data,labels,images,outputs
			# Display all accuracies
			for i in range(len(scene_classes)):
				print('Peripheral Bias Accuracy %5s: %2d %%' % (scene_classes[i], 100 * float(class_periphery_Object[i]) / float(class_total_Object[i])))
				print('Fovea Bias Accuracy %5s: %2d %%' % (scene_classes[i], 100 * float(class_fovea_Object[i]) / float(class_total_Object[i])))
