...
#Training_Regimes = ['Reference-Net']
...
Name_Regimes = ['Reference','Foveation-Texture','Uniform-Blur','Foveation-Blur']

Training_Name_Legend = dict(zip(All_Regimes,Name_Regimes))


Experiment_name_mod = ['Window_Cue_Conflict']
...
Experiment_name_in = ['Window_Cue_Conflict']
Experiment_name_out = ['Window_Cue_Conflict']

Experiment_Name_Legend = dict(zip(Experiment_name_in,Experiment_name_out))

Acc_Regimes_Periphery = np.zeros((len(Training_Regimes),number_runs,len(Experiment_name_mod),8))
Acc_Regimes_Fovea = np.zeros((len(Training_Regimes),number_runs,len(Experiment_name_mod),8))
...
print(Acc_Regimes_Periphery_mean)
print(Acc_Regimes_Periphery_std)

print(Acc_Regimes_Fovea_mean)
print(Acc_Regimes_Fovea_std)
...
for i in range(len(Experiment_name_mod)):
	...
	for j in range(len(Training_Regimes)):
		axs[x_i,y_i].errorbar(x,Acc_Regimes_Periphery_mean[j,i,:],yerr=Acc_Regimes_Periphery_std[j,i,:],markersize=10,marker='o',color=Color_Legend[Training_Regimes[j]],alpha=Alpha_Legend[Training_Regimes[j]],linestyle=Line_Legend[Training_Regimes[j]],linewidth=2)
	#
	...
	#
	for j in range(len(Training_Regimes)):
		axs[x_i,y_i].errorbar(x,Acc_Regimes_Fovea_mean[j,i,:],yerr=Acc_Regimes_Fovea_std[j,i,:],markersize=10,marker='*', color=Color_Legend[Training_Regimes[j]],alpha=Alpha_Legend[Training_Regimes[j]],linestyle=Line_Legend[Training_Regimes[j]],linewidth=2)
	#
	axs[x_i,y_i].plot(x,np.repeat(Top_k/20.0*100.0,8),'--y',linewidth=2)
...
for i in range(len(Experiment_name_mod)):
	...
	axs[x_i,y_i].set_title(Experiment_Name_Legend[Experiment_name_mod[i]])
	axs[x_i,y_i].set_xticks(np.arange(0,1.1,step=0.1))
	axs[x_i,y_i].set_ylabel('("o") Peripheral Scene Classification Bias')
	ax2 = axs[x_i,y_i].twinx() # instantiate a second axes that shares the same x-axis
	ax2.set_ylabel('("*") Foveal Scene Classification Bias')
	ax2.set(xlim=(-0.1,1.1),ylim=(0,Upper_lim))
	axs[x_i,y_i].set_xlabel('Percentage of Central Image Area with Foveal Class')
