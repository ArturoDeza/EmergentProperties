...
# Example line:
#python compute_Occlusion.py AlexNet Reference-Net 1 1 1 120

# Model_Type = 'AlexNet'
# Metamer_Type = 'Reference-Net'
# top_k_num = 1
# start_run_id = 1
# end_run_id = 1
# epoch = 120
...
Distortion_Family = ['Scotoma','Glaucoma','Left2Right','Top2Bottom']
#Distortion_Family = ['Left2Right','Top2Bottom']
Distortion_Steps = [17,17,17,17]
...
for run_id in range(start_run_id,end_run_id+1):
	...
	for m in range(len(Distortion_Family)):
		...
		for z in range(Distortion_Steps[m]):
			##########################################
			...
			testloader = torch.utils.data.DataLoader(testset,batch_size=testing_batch_size,shuffle=False,num_workers=6,pin_memory=True)
			...
			for i in range(len(scene_classes)):
				print('Accuracy %5s: %2d %%' % (scene_classes[i], 100 * float(class_correct_Object[i]) / float(class_total_Object[i])))
			...
			class_total_Object_str = './Results/Top' + top_k_num_str + '/' + Metamer_Type + '/' + Distortion_Type + '/' + Model_Type + '_' + run_id_str + '_class_total_Object_' + z_str + '_Epoch_' + epoch_str_save + '.npy'
